...
const IDX_REWARD_POINTS_TRANSACTION_TYPE_USER_ID = 'idx_reward_points_transaction__type_user_id';
...
export async function upgrade(queryFn: (query: string, values?: any[]) => Promise<any[]>): Promise<void> {
  await queryFn(`
    ALTER TABLE \`${DbTables.REWARD_POINTS_TRANSACTION}\`
      ...
      ADD INDEX \`${IDX_REWARD_POINTS_TRANSACTION_TYPE_USER_ID}\` (\`type\` ASC, \`user_id\` ASC, \`createTime\` ASC) VISIBLE,
...
export async function downgrade(queryFn: (query: string, values?: any[]) => Promise<any[]>): Promise<void> {
  await queryFn(`
    ALTER TABLE \`${DbTables.REWARD_POINTS_TRANSACTION}\`
      ...
      DROP INDEX \`${IDX_REWARD_POINTS_TRANSACTION_TYPE_USER_ID}\`;
  `);
}
